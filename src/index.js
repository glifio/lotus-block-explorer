...
class Lotus {
  ...
  viewBlock = async block => {
    ...
    // avoid mutating fn params
    const newBlock = { ...block }
    newBlock.Messages = messages
    newBlock.messageReceipts = parentMessageReceipts
      ? parentMessageReceipts.map(shapeMessageReceipt)
      : []
    // mark this block as seen
    return this.cacheBlock(newBlock)
  }
  ...
  explore = async (options = {}) => {
    ...
    for (let i = this.fromHeight; i < this.toHeight; i += 1) {
      this.blocksToView.push(...(await this.getNextBlocksFromTipsetByHeight(i)))
    }
  ...
  loadNextBlocks = async num => {
    ...
    for (let i = prevFromHeight - num; i < prevFromHeight; i += 1) {
      this.blocksToView.push(...(await this.getNextBlocksFromTipsetByHeight(i)))
    }
